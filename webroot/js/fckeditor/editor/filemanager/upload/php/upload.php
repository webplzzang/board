...
$temp = str_replace(".","",$temp);
$temp = str_replace("/","",$temp);
$temp = strtoupper(substr($temp,3,15));
$name_chk = explode(".",$oFile['name']);
$sFileName = $temp.".".$name_chk[sizeof($name_chk) - 1] ;
...
if ( isset( $Config['UserFilesAbsolutePath'] ) && strlen( $Config['UserFilesAbsolutePath'] ) > 0 )
	$sServerDir = $Config['UserFilesAbsolutePath'].date("Ymd",time())."/";
else 
	$sServerDir = GetRootPath() . $Config["UserFilesPath"].date("Ymd",time())."/";
...
while ( true )
{
	...
	else
	{
		...
		if ( $Config['UseFileType'] )
			$sFileUrl = $Config["UserFilesPath"].date("Ymd",time()) . "/".$sType . '/' . $sFileName ;
		else
			$sFileUrl = $Config["UserFilesPath"].date("Ymd",time()) . "/".$sFileName ;
